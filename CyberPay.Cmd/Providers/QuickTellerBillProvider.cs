...
namespace CyberPay.Cmd.Providers
{
    ...
    public class QuickTellerBillProvider : IQuickTellerBillProvider
    {
        ...
        public List<QuickTellerBanksViewModel> GetBanksCodes()
        {
            string billresponse = this.SendRequest("", $"{ ConfigurationManager.AppSettings["QuicktellerUrl"]}/configuration/fundstransferbanks", "GET");
            var responseobject = JsonConvert.DeserializeObject<QuicktellerServiceJSONResponse>(billresponse);
            if (responseobject == null)
            {
                responseobject = new QuicktellerServiceJSONResponse();
            }
            var bankscodes = responseobject.BankCodes;
            if (bankscodes == null)
            {
                responseobject.BankCodes = new List<QuickTellerBanksViewModel>();
            }
